...
class ProjectManager:
    ...
    def sync_from_cloud(self, project_id: str) -> bool:
        ...
        if ok:
            # BUGFIX (mất dữ liệu "cloud save"): trước đây luôn gọi thẳng
            # _create_project_meta(), hàm này TẠO MỚI metadata với title/
            # plot_summary/reference_urls/created_at rỗng/mặc định rồi GHI
            # ĐÈ lên "_project_meta.json" — kể cả khi download_project() ở
            # trên vừa tải về đúng file "_project_meta.json" thật (sau khi
            # cloud_storage.py được sửa để không còn bỏ qua nó). Giờ ưu tiên
            # đọc lại file vừa tải về (nếu có) và chỉ làm mới các trường suy
            # ra được từ đĩa (status, stages_completed...), giữ nguyên các
            # trường người dùng đã nhập 1 lần lúc tạo project.
            existing_meta = self._load_project_meta(project_dir)
            if existing_meta is not None:
                self._refresh_project_meta(project_dir, existing_meta)
            else:
                # Project cũ chưa từng có "_project_meta.json" trên cloud
                # (được tạo/sync trước khi bugfix này tồn tại) -> đành tạo
                # mới như cũ, không có gì để phục hồi.
                self._create_project_meta(project_dir)
            print(f"[cloud] Tải xong: {project_dir}")
        return ok

    def prune_old_projects(self, keep: int = 1, prune_cloud: bool = True,
                            protect: list[str] | None = None) -> list[str]:
        """Tự động xoá project cũ, chỉ giữ lại `keep` project được sửa gần
        đây nhất (local + cloud, dựa trên "last_modified" thật — cùng tiêu
        chí đang dùng để hiển thị/sắp xếp trong list_all_projects()).

        Tính năng này phục vụ yêu cầu "chỉ muốn 1 project mới nhất tồn tại
        tại 1 thời điểm" — hữu ích khi chạy trên môi trường tính phí theo
        dung lượng cloud, hoặc đơn giản là không muốn project cũ bị bỏ quên
        mãi trên storage. Mặc định BẬT (xem config.toml [project]
        auto_prune_old_projects), có thể tắt hẳn bằng cách đặt giá trị đó
        thành false.

        An toàn:
          - Không bao giờ xoá quá (tổng số project - keep) project — nếu số
            project hiện có <= keep thì không làm gì cả.
          - keep < 1 được ép về 1 (không cho phép "xoá sạch mọi project").
          - Mỗi lần xoá được bọc try/except riêng lẻ: 1 project xoá lỗi
            (vd mất mạng giữa chừng khi xoá trên cloud) không làm dừng việc
            xoá các project khác.
          - project_id không xác định được (thiếu trong metadata) bị bỏ qua,
            không đoán mò.

        `protect`: danh sách project_id KHÔNG BAO GIỜ bị xoá bởi lần gọi
        này, dù không nằm trong top `keep` theo last_modified — dùng để bảo
        vệ project sắp/đang được chạy (vd ci_run_latest_project.py vừa chọn
        1 project "in_progress" để tiếp tục, nhưng có 1 project "completed"
        khác lại có last_modified mới hơn — nếu không có `protect`, project
        sắp chạy có thể bị tự xoá nhầm trước khi kịp dùng tới).

        Trả về danh sách project_id đã xoá thành công (rỗng nếu không có gì
        để xoá).
        """
        if keep < 1:
            keep = 1
        ...
        projects = self.list_all_projects(include_cloud=prune_cloud and self.cloud is not None)
        if len(projects) <= keep:
            return []

        # Cùng cách sắp xếp với display_projects()/pick_next_project trong
        # ci_run_latest_project.py: chuỗi "YYYY-MM-DD HH:MM:SS" so sánh trực
        # tiếp được vì đã ở dạng ISO-like.
        projects.sort(key=lambda p: p.get("last_modified") or "", reverse=True)
        # Project được "protect" không đếm vào quota `keep` và không bao giờ
        # nằm trong danh sách xoá — chỉ tính top `keep` trong PHẦN CÒN LẠI.
        rest = [p for p in projects if p.get("project_id") not in protected]
        to_delete = rest[keep:]
        ...
        for p in to_delete:
            ...
            if not project_id:
                continue
            try:
                if p.get("source") == "cloud_only":
                    # Không có bản cục bộ để xoá qua delete_project() —
                    # chỉ xoá trên cloud (nếu được phép).
                    if prune_cloud and self.cloud and self.cloud.delete_project(project_id):
                        deleted_ids.append(project_id)
                else:
                    if self.delete_project(project_id, cloud=prune_cloud and self.cloud is not None):
                        deleted_ids.append(project_id)
            except Exception as exc:
                print(f"[project] Không tự xoá được project cũ '{project_id}': {exc}")

        if deleted_ids:
            print(f"[project] Auto-prune: đã xoá {len(deleted_ids)} project cũ "
                  f"(chỉ giữ {keep} project mới nhất): {deleted_ids}")
        return deleted_ids
