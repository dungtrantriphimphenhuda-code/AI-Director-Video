...
def _required_packages_for_backends(cfg) -> dict[str, str]:
    """Trả về map {module_name: pip_name} cho các backend ĐANG ĐƯỢC CHỌN trong
    config.toml — KHÔNG cài toàn bộ mọi thứ như trước nữa.

    Chỉ các package của backend thực sự được bật mới được kiểm tra/cài đặt:
      - ASR:   processing.asr_backend ("funasr" | "whisper")
      - Vision: processing.vision_backend ("local" | "cerebras" | "mistral" |
               "moondream" | "gemma4_video")
      - Script: api.script_backend ("local" | "cerebras")
      - TTS:   tts.engine ("piper" | "edge-tts" | "gemini")
      - Cloud: cloud.enabled
    ...
    checks: dict[str, str] = {
        # Core — bất kỳ pipeline nào cũng chạy preprocess + render.
    ...
    # ---- ASR ----
    asr_backend = (cfg.get("processing.asr_backend", "funasr") or "funasr").strip().lower()
    if asr_backend == "funasr":
        # FunASR là backend chính; faster-whisper vẫn được giữ làm phương án
        # dự phòng tự động (xem asr.py run_asr) nên cần cài cả hai.
        checks["funasr"] = "funasr"
        checks["faster_whisper"] = "faster-whisper"
    else:
        checks["faster_whisper"] = "faster-whisper"

    # ---- Vision ----
    vision_backend = (cfg.get("processing.vision_backend", "gemma4_video") or "gemma4_video").strip().lower()
    if vision_backend in ("local", "moondream"):
        # Hai backend chạy model trên máy qua transformers (Qwen3-VL / Moondream2).
        checks.update({
        ...
        if vision_backend == "local":
            checks["torchvision"] = "torchvision"
            checks["accelerate"] = "accelerate"
            checks["qwen_vl_utils"] = "qwen-vl-utils"
            checks["av"] = "av"
    elif vision_backend in ("cerebras", "mistral"):
        # Hai backend API này dùng client OpenAI-compatible.
        checks["openai"] = "openai"
    # vision_backend == "gemma4_video": chạy qua Ollama (HTTP), chỉ cần
    # requests + opencv/PIL (đều đã ở core) để trích frame — không cần thêm.

    # ---- Script writer ----
    script_backend = (cfg.get("api.script_backend", "local") or "local").strip().lower()
    if script_backend == "local":
        checks["llama_cpp"] = "llama-cpp-python"
        checks["huggingface_hub"] = "huggingface_hub"
    else:
        checks["openai"] = "openai"

    # ---- TTS ----
    tts_engine = (cfg.get("tts.engine", "piper") or "piper").strip().lower()
    if tts_engine == "piper":
        checks["piper"] = "piper-tts"
    elif tts_engine == "edge-tts":
        checks["edge_tts"] = "edge-tts"
    elif tts_engine == "gemini":
        # __import__("google.genai") — nếu chỉ có namespace package `google`
        # (đi kèm thư viện khác) mà chưa có google-genai thì vẫn ImportError.
        checks["google.genai"] = "google-genai"

    # ---- Cloud ----
    if cfg.get("cloud.enabled", False):
        checks["boto3"] = "boto3"

    # ---- Reference (transcript video đối thủ) ----
    if cfg.get("reference.urls"):
        checks["youtube_transcript_api"] = "youtube-transcript-api"
        if cfg.get("reference.use_ytdlp_fallback", True):
            checks["yt_dlp"] = "yt-dlp"

    return checks


def ensure_python_packages(cfg=None) -> None:
    """Kiểm tra và cài đặt các package cần thiết theo backend đang chọn.

    Khác với bản cũ (cài gần như toàn bộ mọi thứ cùng lúc), hàm này chỉ kiểm
    tra/cài những package thuộc backend thực sự được bật trong config.toml —
    đúng tinh thần lazy-load: thư viện nặng (torch/transformers/funasr/llama-
    cpp...) chỉ được cài khi stage tương ứng thực sự chạy.
    """
    checks = _required_packages_for_backends(cfg)

    missing = []
    print(f"[deps] Đang kiểm tra {len(checks)} package theo backend trong config.toml...")
    ...
    if missing:
        ...
        print("[deps] Tất cả package cần thiết đã sẵn sàng.")
    ...
    while True:
        ...
        if action == "0" or action is None:
            ...
            try:
                ...
                # Tự động xoá project cũ, chỉ giữ lại project vừa tạo (+ các
                # project khác nếu keep_projects_count > 1) — xem config.toml
                # [project] auto_prune_old_projects.
                if cfg.get("project.auto_prune_old_projects", True):
                    keep_n = max(1, int(cfg.get("project.keep_projects_count", 1)))
                    pm.prune_old_projects(keep=keep_n, prune_cloud=cloud is not None,
                                           protect=[project_id])
...
def run_pipeline_on_project(cfg, pm: ProjectManager, project_id: str, cloud: CloudStorage | None) -> None:
    ...
    vision_analysis = run_stage("vision", lambda: vision.run_vision_analysis(cfg, preprocess_result, ckpt, asr_timeline))
    ...
    # Tự động xoá project cũ, chỉ giữ lại "keep_projects_count" project mới
    # nhất (mặc định 1) — xem config.toml [project] auto_prune_old_projects.
    # Chạy SAU sync cuối cùng để project vừa hoàn tất/vừa chạy chắc chắn có
    # last_modified mới nhất, không bị chính mình xoá nhầm.
    if cfg.get("project.auto_prune_old_projects", True):
        keep_n = max(1, int(cfg.get("project.keep_projects_count", 1)))
        pm.prune_old_projects(keep=keep_n, prune_cloud=cloud is not None,
                               protect=[project_id])
...
def main() -> None:
    ...
    # Khởi tạo cloud storage dùng boto3 (S3-compatible, Tigris/Filebase/R2...)
