...
def main() -> int:
    ...
    # Cài (nếu thiếu) ĐÚNG package cho backend đang chọn trong config.toml.
    # Hàm này backend-aware nên trên CI — nơi config.py ép các backend nặng
    # sang API nhẹ (asr->whisper, vision->mistral, script->cerebras) — chỉ
    # pip-install thêm faster-whisper + openai thay vì cả stack torch/
    # transformers nặng nề.
    run_module.ensure_python_packages(cfg)
    ...
    # Tự động xoá project cũ NGAY sau khi chọn (không đợi tới cuối
    # run_pipeline_on_project) — trên CI, job thường bị GitHub Actions huỷ
    # giữa chừng (hết hạn mức 6h/job) trước khi kịp chạy tới đoạn dọn dẹp ở
    # cuối run_pipeline_on_project, nên dọn ở đây sớm hơn để đáng tin cậy
    # hơn. project_id vừa chọn (mới nhất) không bị ảnh hưởng vì nó luôn nằm
    # trong top "keep_projects_count" theo last_modified.
