...
_CI_LIGHTWEIGHT_OVERRIDES = [
    ...
    ("processing.vision_backend", {
        ...
        # KHÔNG còn tự đổi "gemma4_video" sang mistral trên CI nữa: kể từ bản
        # này, backend gemma4_video chạy QUA OLLAMA (model `gemma4:12b-it-qat`
        # ~7GB GGUF Q4_0, llama.cpp tối ưu CPU) — cơ chế đã kiểm chứng chạy
        # THÀNH CÔNG trên runner GitHub Actions ở dự án fork OpenManus
        # (handandfeet-agent), workflow .github/workflows/run-pipeline.yml tự
        # cài Ollama + pull model. Không còn tải file safetensors ~24GB qua
        # transformers nữa (vốn là thứ làm runner Actions chết với
        # "unable to mmap ... Cannot allocate memory (12)").
    }),
