...
Hỗ trợ 3 engine, chọn qua `tts.engine` trong config.toml:
  - "piper" (mặc định): model TTS LOCAL cực nhẹ (VITS, ~15-60MB tuỳ giọng,
    chạy qua ONNX Runtime CPU) — KHÔNG cần GPU, KHÔNG cần venv riêng, KHÔNG
    phụ thuộc mạng lúc chạy (chỉ tải giọng ~vài chục MB một lần đầu), tốc độ
    gần như real-time ngay cả trên CPU yếu — chạy tốt trên GitHub Actions
    runner miễn phí. Chất lượng giọng ở mức "nghe hiểu tốt, rõ ràng, dễ
    nghe", không tự nhiên/biểu cảm bằng Gemini, nhưng đơn giản và ổn định
    nhất — không bị lỗi mạng như edge-tts, không bị vướng xung đột phiên
    bản Python/package nặng nề như engine local cũ (viterbox, đã gỡ bỏ).
    Không hỗ trợ voice cloning.
  - "edge-tts": dùng edge-tts, không cần API key, không cần GPU. LƯU Ý: đây
    là API không chính thức của Microsoft, thỉnh thoảng bị chặn/rớt kết nối
    khi chạy từ IP datacenter (vd GitHub Actions) — lỗi
...
song. piper chạy đồng bộ tuần tự (model ONNX cực nhẹ, mỗi câu chỉ mất một
phần nhỏ của giây trên CPU nên không cần song song hoá).
"""
...
async def _synthesize_one_with_retry(
    ...
    try:
        ...
        raise ImportError(
            "Engine TTS 'edge-tts' cần package 'edge-tts' — chạy: "
...
def _synthesize_all_piper(
    ...
    cảm bằng Gemini TTS, nhưng đủ rõ ràng,
...
def run_tts(cfg, storyboard: dict[str, Any], checkpoint_mgr=None) -> dict[str, Any]:
    ...
    if engine not in ("piper", "edge-tts", "gemini"):
        print(f"[tts] CẢNH BÁO: tts.engine = '{engine}' trong config.toml không hợp lệ "
              f"(chỉ hỗ trợ 'piper', 'edge-tts' hoặc 'gemini') — dùng 'piper' mặc định.")
        engine = "piper"
