> **Ghi chú (bản Colab):** file này mô tả dependency của bản skill Claude Code gốc,
> bao gồm `.env` và đường dẫn Windows cứng (`D:\models\...`). Bản code Python độc lập
> trong repo này KHÔNG dùng `.env`/`os.getenv` và KHÔNG hỗ trợ nhánh Volcengine — mọi
> cấu hình nằm trong `config.toml`, xem `README_COLAB.md`. Giữ file này lại chỉ để tham
> khảo lịch sử thiết kế ASR/Qwen-VL gốc.
