...
def resync_output_to_tts_actual(
    storyboard: dict[str, Any],
    tts_audio_map: dict[str, Path],
) -> dict[str, Any]:
    """
    BUGFIX (thời lượng TTS thật > slot ước tính -> audio bị cắt cụt):

    clip["output"]["start"/"end"] trước đây chỉ được tính MỘT LẦN ở
    script_writer.build_storyboard, dựa trên ƯỚC TÍNH calc_output_duration
    (số từ / words_per_sec) TRƯỚC KHI TTS thật sự chạy. tts.py có đo lại
    thời lượng thật (actual_tts_sec trong tts_report.json) nhưng chỉ để log,
    KHÔNG ghi ngược lại vào storyboard. Do đó dù words_per_sec trong
    config.toml có đo đúng cỡ nào, sai số từng câu vẫn khiến render_clip()
    cắt cứng TTS theo out_dur cũ (xem cmd_mix "-t" str(out_dur)) bất cứ khi
    nào audio thật dài hơn ước tính -> giọng đọc bị hụt giữa câu.

    Hàm này ĐO LẠI actual duration của từng file TTS bằng ffprobe (chính xác
    tuyệt đối, không còn là ước tính) và ghi đè output.start/end tuần tự,
    cộng dồn không khoảng trống — làm NGAY TRƯỚC khi render, để mọi phép
    tính sau đó (speed clamp, mix cutoff, validate) đều dựa trên số thật.
    Clip nào không có audio TTS (tts_audio_map thiếu) thì giữ nguyên
    out_dur ước tính cũ.
    """
    cursor = 0.0
    for clip in storyboard["timeline"]:
        old_dur = clip["output"]["end"] - clip["output"]["start"]
        audio_path = tts_audio_map.get(clip["clip_id"])
        if audio_path is not None and Path(audio_path).exists():
            actual_dur = _ffprobe_duration(Path(audio_path))
            if abs(actual_dur - old_dur) > 0.15:
                print(
                    f"[render] {clip['clip_id']}: slot ước tính {old_dur:.2f}s lệch so với "
                    f"TTS thật {actual_dur:.2f}s (chênh {actual_dur - old_dur:+.2f}s) -> "
                    "đồng bộ lại theo thời lượng thật trước khi render."
                )
            dur = actual_dur
        else:
            dur = old_dur
        clip["output"] = {"start": round(cursor, 3), "end": round(cursor + dur, 3)}
        cursor += dur
    return storyboard
...
def run_render(cfg, storyboard: dict[str, Any], tts_result: dict[str, Any] | None, checkpoint_mgr=None) -> dict[str, Any]:
    ...
    if tts_audio_map:
        print("[render] Đồng bộ lại output timeline theo thời lượng TTS thật (ffprobe)...")
        storyboard = resync_output_to_tts_actual(storyboard, tts_audio_map)
