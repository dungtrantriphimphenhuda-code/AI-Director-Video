...
boto3 = None  # noqa: N816 — gán thật trong _ensure_boto3_imported() bên dưới
Config = None


def _ensure_boto3_imported() -> bool:
    """Thử import boto3 (và botocore.config.Config); trả về True nếu dùng được.

    BUGFIX: trước đây HAS_BOTO3 chỉ được tính DUY NHẤT 1 LẦN lúc module này
    được nạp — thường là ngay khi run.py chạy dòng
    `from cloud_storage import ...` ở ĐẦU file, tức là TRƯỚC KHI
    ensure_python_packages() trong run.py kịp `pip install boto3`. Vì Python
    cache module đã import, biến cờ HAS_BOTO3 = False đó bị "đóng băng" cho
    suốt phiên chạy hiện tại, dù ngay sau đó pip cài boto3 thành công thật —
    khiến get_cloud_storage_from_config() vẫn báo "[cloud] Chưa cài boto3"
    dù log pip ngay phía trên ghi rõ "Successfully installed boto3-...".
    pip cài package vào ổ đĩa, nhưng KHÔNG tự làm process Python đang chạy
    "thấy" module mới có sẵn — phải import (lại) mới nạp được.

    Hàm này được gọi LẠI mỗi lần cần dùng boto3 (CloudStorage.__init__),
    thay vì đọc 1 cờ tính sẵn — nên nếu lần trước thất bại (chưa cài) nhưng
    từ đó tới giờ đã được pip cài thêm trong CÙNG process, lần gọi sau sẽ
    thấy import thành công ngay, không cần khởi động lại runtime/kernel.
    """
    global boto3, Config
...
# Thử import ngay lúc nạp module (như trước) để các nơi khác import HAS_BOTO3
# từ module này (nếu có) vẫn thấy giá trị hợp lý ngay từ đầu — nhưng đây CHỈ
# còn là giá trị tham khảo ban đầu, không phải nguồn chân lý duy nhất nữa.
# Mọi chỗ cần biết boto3 có dùng được KHÔNG phải gọi _ensure_boto3_imported()
# để được kiểm tra lại (và tự phục hồi nếu vừa được cài thêm), KHÔNG đọc
# thẳng biến HAS_BOTO3 này.
HAS_BOTO3 = _ensure_boto3_imported()
...
class CloudStorage:
    ...
    def upload_project(self, project_dir: Path, project_id: str, max_workers: int = 16) -> dict[str, Any]:
        ...
        for local_file in project_dir.rglob("*"):
            if local_file.is_dir():
                continue
            # BUGFIX (mất dữ liệu "cloud save"): trước đây MỌI file bắt đầu
            # bằng "_" và kết thúc bằng ".json" đều bị bỏ qua khi upload —
            # bao gồm cả "_project_meta.json", nơi lưu title/plot_summary/
            # reference_urls/created_at mà người dùng nhập MỘT LẦN lúc tạo
            # project. Hậu quả: trên môi trường không giữ ổ đĩa giữa các lần
            # chạy (vd GitHub Actions — runner bị xoá sạch sau mỗi job), mỗi
            # lần tải project về từ cloud, file này không tồn tại -> title/
            # plot_summary/reference_urls bị mất VĨNH VIỄN sau lần chạy đầu
            # tiên (bị ghi đè về giá trị mặc định rỗng), khiến script được
            # tạo ra không còn dùng đúng tóm tắt cốt truyện / link tham khảo
            # người dùng đã cung cấp. Các file "_*.json" khác (vd
            # "_emergency_stop.json", "_upload_meta.json") vẫn cố tình bỏ qua
            # vì chúng chỉ là marker nội bộ, không phải dữ liệu project.
            if (local_file.name.startswith("_") and local_file.name.endswith(".json")
                    and local_file.name != "_project_meta.json"):
                continue
    ...
    def download_project(self, project_id: str, local_dir: Path, max_workers: int = 16) -> bool:
        ...
        for remote_key, size in files:
            ...
            if not rel:
                continue
            # BUGFIX: xem giải thích ở upload_project() — trước đây MỌI key
            # có rel bắt đầu bằng "_" bị bỏ qua khi tải về, nên dù có sửa
            # upload_project() để đẩy "_project_meta.json" lên cloud, file đó
            # vẫn sẽ KHÔNG BAO GIỜ được tải về lại. Giờ chỉ bỏ qua các key
            # "_*" khác (marker nội bộ), giữ lại "_project_meta.json".
            if rel.startswith("_") and rel != "_project_meta.json":
                continue
